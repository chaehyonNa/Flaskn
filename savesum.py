...
def saveSum(names,amounts):
    ...
    if value1==None:
        sql = "INSERT INTO impo (date, kacl, carbo, province, protein) VALUES (%s, %s, %s, %s, %s)"
        cursor.execute(sql,(date, kacl, carbo, province, protein))

    if value1[1]==date:
        sql = "UPDATE impo SET kacl=%s, carbo=%s, province=%s, protein=%s  WHERE date =%s "
        cursor.execute(sql, ((value1[2]+kacl), (value1[3]+carbo), (value1[4]+province), (value1[5]+protein), value1[1]))
    elif value1[1]!=date:
        sql = "INSERT INTO impo (date, kacl, carbo, province, protein) VALUES (%s, %s, %s, %s, %s)"
        cursor.execute(sql,(date, kacl, carbo, province, protein))

    sql1 =  "SELECT id FROM project01.impo ORDER BY id desc LIMIT 7; "
    cursor.execute(sql1)
    num = cursor.fetchall()
    a=[]

    for i in num:
        cursor.execute(f'SELECT * FROM impo WHERE id={i[0]};')
        value2 = cursor.fetchone()
        b = {
                'date': value2[1],
                'kacl': value2[2],
    ...
    if len(num1)>7:
        sql="DELETE FROM impo WHERE id = %s"
